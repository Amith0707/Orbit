import { HugeiconsIcon } from "@hugeicons/react";
import { SparklesIcon, UserGroupIcon } from "@hugeicons/core-free-icons";
import { Skeleton } from "@/components/ui/skeleton";
import { EmptyState } from "@/components/composite/EmptyState";
import { useCommunityRecommendations } from "../hooks/useRecommendations";
import { CommunityRecommendationCard } from "./CommunityRecommendationCard";

export function RecommendedCommunitiesRow() {
  const { data, isPending } = useCommunityRecommendations();

  return (
    <section className="space-y-3">
      <div className="flex items-center gap-2">
        <HugeiconsIcon icon={SparklesIcon} strokeWidth={2} className="size-4 text-ai-accent" />
        <h2 className="font-heading text-sm font-semibold">Recommended for you</h2>
      </div>
      {isPending ? (
        <div className="flex gap-4 overflow-hidden">
          {[0, 1, 2].map((i) => (
            <Skeleton key={i} className="h-56 w-72 shrink-0 rounded-2xl" />
          ))}
        </div>
      ) : !data || data.length === 0 ? (
        <EmptyState
          icon={<HugeiconsIcon icon={UserGroupIcon} strokeWidth={2} className="size-5" />}
          title="No new recommendations right now"
          description="Add a few interests to your profile and we'll surface communities worth joining."
        />
      ) : (
        <div className="flex gap-4 overflow-x-auto pb-2">
          {data.map((rec) => (
            <CommunityRecommendationCard key={rec.communityId} recommendation={rec} />
          ))}
        </div>
      )}
    </section>
  );
}
