...
import { Chess01Icon, RobotIcon, UserMultiple02Icon, RefreshIcon } from "@hugeicons/core-free-icons";
...
import { ChessBoardGame } from "@/features/games/components/ChessBoardGame";
import type { ChessState, Match } from "@/features/games/api/games";
...
export default function ChessGamePage() {
  ...
  const startMatch = async (mode: "pvp" | "pvai") => {
    try {
      const created = await createMatch.mutateAsync({ gameKey: "chess", mode });
  ...
  const resultLabel = (result: string | null) => {
    ...
    if (result === "player_one_win") return match?.mode === "pvai" ? "You won!" : "White wins!";
    if (result === "player_two_win") return match?.mode === "pvai" ? "The AI won this time." : "Black wins!";
    return null;
  };
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6 p-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="font-heading text-xl font-semibold">Chess</h1>
          <p className="text-sm text-muted-foreground">Play a coworker locally or challenge the AI.</p>
        </div>
      ...
      {!matchId ? (
        <Card>
          <CardContent className="flex flex-col items-center gap-4 py-10 text-center">
            <div className="flex size-14 items-center justify-center rounded-2xl bg-muted text-muted-foreground">
              <HugeiconsIcon icon={Chess01Icon} strokeWidth={1.5} className="size-7" />
            </div>
        ...
        <div className="space-y-4">
          ...
          <ChessBoardGame match={match as unknown as Match<ChessState>} />
          {match.status === "completed" && (
